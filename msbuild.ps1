...
if ($Type -eq "free") {
	Run-MSBuild $solutionpath "ivc.sln" $configuration["free"] $platform[$Arch]
}
elseif ($Type -eq "checked") {
	Run-MSBuild $solutionpath "ivc.sln" $configuration["checked"] $platform[$Arch]
}
elseif ($Type -eq "sdv") {
	$archivepath = "ivc"
	...
	Run-MSBuildSDV $solutionpath "ivc" $configuration["sdv"] $platform[$Arch]
